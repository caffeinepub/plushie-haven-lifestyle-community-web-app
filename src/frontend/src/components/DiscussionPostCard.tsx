...
import { DiscussionPost } from '../backend';
import { useGetDiscussionLikeCount, useHasUserLikedDiscussion, useToggleDiscussionLike, useGetDiscussionComments, useGetUserProfile } from '../hooks/useQueries';
...
export default function DiscussionPostCard({ post }: DiscussionPostCardProps) {
  ...
  const { data: likeCount = 0n } = useGetDiscussionLikeCount(post.id);
  const { data: hasLiked = false } = useHasUserLikedDiscussion(post.id);
  const { data: comments = [] } = useGetDiscussionComments(post.id);
  ...
  return (
    <article className="rounded-3xl bg-white dark:bg-gray-800 p-6 shadow-lg transition-shadow hover:shadow-xl">
      ...
      {/* Comments Section */}
      {showComments && <DiscussionCommentsSection postId={post.id} />}
    </article>
  );
}
