...
import { GroupComment } from '../backend';
import { useGetGroupComments, useCreateGroupComment, useGetUserProfile } from '../hooks/useQueries';
...
function GroupCommentItem({ comment }: { comment: GroupComment }) {
...
export default function GroupCommentsSection({ postId, groupId }: GroupCommentsSectionProps) {
  const { identity } = useInternetIdentity();
  const [commentText, setCommentText] = useState('');
  const { data: comments = [], isLoading, refetch } = useGetGroupComments(postId);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refetch comments to ensure UI is updated
      await refetch();
    } catch (error: any) {
      console.error('Create comment error:', error);
      if (error.message?.includes('Unauthorized') || error.message?.includes('Only group members')) {
        toast.error('Only group members can comment on this post');
      } else {
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {isLoading ? (
          ...
          comments.map((comment) => (
            <GroupCommentItem key={comment.id} comment={comment} />
          ))
        ) : (
