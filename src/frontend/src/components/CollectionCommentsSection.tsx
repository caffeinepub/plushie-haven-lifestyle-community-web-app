...
import { useGetCollectionComments, useCreateCollectionComment, useGetUserProfile } from '../hooks/useQueries';
...
interface CollectionCommentsSectionProps {
  postId: string;
}

export default function CollectionCommentsSection({ postId }: CollectionCommentsSectionProps) {
  const { identity } = useInternetIdentity();
  const [commentText, setCommentText] = useState('');
  const { data: comments = [], isLoading } = useGetCollectionComments(postId);
  const createComment = useCreateCollectionComment();
